import Image from "next/image";
import BotaoCannaDocs from "./button";

export default function CtaCannaDocs() {
    return (
        <div className="bg-gray-50">
            <div className="mx-auto max-w-9xl py-24 sm:py-32 ">
                <div className="relative isolate overflow-hidden bg-cyan-500 px-6 pt-16 shadow-2xl sm:rounded-3xl sm:px-16 md:pt-24 lg:flex lg:gap-x-20 lg:px-24 lg:pt-0">
                    <svg
                        viewBox="0 0 1024 1024"
                        className="absolute left-1/2 top-1/2 -z-10 h-[64rem] w-[64rem] -translate-y-1/2 [mask-image:radial-gradient(closest-side,white,transparent)] sm:left-full sm:-ml-80 lg:left-1/2 lg:ml-0 lg:-translate-x-1/2 lg:translate-y-0"
                        aria-hidden="true"
                    >
                        <circle cx={512} cy={512} r={512} fill="url(#759c1415-0410-454c-8f7c-9a820de03641)" fillOpacity="0.7" />
                        <defs>
                            <radialGradient id="759c1415-0410-454c-8f7c-9a820de03641">
                                <stop stopColor="#7cda24" />
                                <stop offset={1} stopColor="#7cda24" />
                            </radialGradient>
                        </defs>
                    </svg>
                    <div className="mx-auto max-w-md text-center lg:mx-0 lg:flex-auto lg:py-32 lg:text-left">
                        <h2 className="text-3xl font-bold tracking-tight text-white sm:text-5xl">
                            Descomplique sua busca por artigos e estudos sobre cannabis medicinal. Encontre tudo o que precisa em um só lugar.
                        </h2>

                        <div className="mt-10 flex items-center justify-center gap-x-6 lg:justify-start">
                            <BotaoCannaDocs />

                        </div>
                    </div>
                    <div className="relative mt-16 h-80 lg:mt-8">
                        <Image
                            className="absolute left-0 top-0 w-[57rem] max-w-none rounded-md"
                            src="/cannadocs-exemplo-1.png"
                            alt="App screenshot"
                            width={1824}
                            height={1080}
                        />
                    </div>
                </div>
            </div>
        </div>
    )
}
